...
pub async fn create_list_db(client: &deadpool_postgres::Client, title: &str, category: &str) -> Result<List, ApiError> {
    let stmt = client.prepare("insert into lists(title,category) values($1,$2) returning id, title, category").await?;
    ...
    client.query(&stmt,&[&title.to_string(),&category.to_string()])
                    ...
                    .collect::<Vec<List>>()
                    .pop()
                    .ok_or(ApiError::DBError("Error in creation".into()))
...
pub async fn delete_list_db(client: &deadpool_postgres::Client, id: i32) -> Result<(), ApiError> {
    ...
    let res1 = client.query_one(&stmt1,&[&id])
        .await;
...
pub async fn update_list_db(client: &deadpool_postgres::Client, id: i32, list: UpdateList ) -> Result<List, ApiError> {
